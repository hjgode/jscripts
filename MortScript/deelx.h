...
public:
	CPosixElxT(const wchar_t * posix, int brightleft);
...
// Implementation
//
template <class CHART> CPosixElxT <CHART> :: CPosixElxT(const wchar_t * posix, int brightleft)
{
	m_brightleft = brightleft;
	...
	if     (!wcsncmp(posix, L"alnum:", 6)) m_posixfun = isAlnum ;
	else if(!wcsncmp(posix, L"alpha:", 6)) m_posixfun = isAlpha ;
	else if(!wcsncmp(posix, L"ascii:", 6)) m_posixfun = isAscii ;
	else if(!wcsncmp(posix, L"cntrl:", 6)) m_posixfun = isCntrl ;
	else if(!wcsncmp(posix, L"digit:", 6)) m_posixfun = isDigit ;
	else if(!wcsncmp(posix, L"graph:", 6)) m_posixfun = isGraph ;
	else if(!wcsncmp(posix, L"lower:", 6)) m_posixfun = isLower ;
	else if(!wcsncmp(posix, L"print:", 6)) m_posixfun = isPrint ;
	else if(!wcsncmp(posix, L"punct:", 6)) m_posixfun = isPunct ;
	else if(!wcsncmp(posix, L"space:", 6)) m_posixfun = isSpace ;
	else if(!wcsncmp(posix, L"upper:", 6)) m_posixfun = isUpper ;
	else if(!wcsncmp(posix, L"xdigit:",7)) m_posixfun = isXdigit;
	else if(!wcsncmp(posix, L"blank:", 6)) m_posixfun = isblank ;
	else                                  m_posixfun = 0         ;
}
	...
	case RCHART('['):
		{
			...
			if(curr == CHART_INFO(RCHART(':'), 1))
			{
				CBufferT <wchar_t> posix;

				do {
					posix.Append(((curr.ch & (CHART)0xff) == curr.ch) ? (CHART)curr.ch : 0, 1);
					MoveNext();
				}
				...
				// posix
				return Keep(new CPosixElxT <wchar_t> (posix.GetBuffer(), flags & RIGHTTOLEFT));
			}
			else if(curr == CHART_INFO(RCHART('^'), 1))
			{
				MoveNext(); // skip '^'
